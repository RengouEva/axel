...
export default function NewProductsPageContent({ products }: { products: Product[] }) {
    ...
    <div className="w-full min-h-screen bg-[var(--bg-primary)]">
      <div className="w-full max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <AnimatedDiv fade slideUp className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-[var(--text-link)]/10 text-[var(--text-link)] text-sm font-semibold mb-4"><Sparkles className="w-4 h-4" /> Nouveautés</div>
          <h1 className="text-4xl font-bold text-[var(--text-primary)] mb-2">Nouveautés</h1>
          <p className="text-[var(--text-secondary)]">Les derniers produits ajoutés à notre catalogue</p>
        </AnimatedDiv>
          ...
          {products.map((product, i) => (
            <AnimatedDiv key={product.id} fade slideUp delay={i * 0.05} className="group bg-[var(--bg-primary)] rounded-2xl border-2 border-[var(--border)] hover:border-transparent hover:shadow-axel-xl transition-all duration-500 hover:-translate-y-2 overflow-hidden">
              <Link href={`/produit/${product.slug}`} className="relative aspect-square bg-[var(--bg-secondary)] overflow-hidden block">
                ...
                <div className="absolute top-3 left-3 px-2 py-1 rounded-full bg-white/90 text-xs font-semibold text-[var(--text-link)]">Nouveau</div>
                {product.boosted && (
              ...
              <div className="p-4">
                ...
                <p className="text-xl font-bold text-[var(--text-primary)] mt-1">{product.price.toLocaleString("fr-FR")} F</p>
